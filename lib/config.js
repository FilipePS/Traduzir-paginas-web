    ...
    twpConfig.import = function (newconfig) {
        for (const key in defaultConfig) {
            if (typeof newconfig[key] !== "undefined") {
                let value = toMapIfTypeIsJson(newconfig[key])
 ...
 *
 * ConfigFile Export also does not support Map, he needs an object, for simple processing, we also give it a Json.
 *
