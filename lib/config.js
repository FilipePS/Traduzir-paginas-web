...

{
    ...
    const defaultConfig = {
        ...
        translateTextOverMousehenPressTwice: "no"
    }
    ...
    twpConfig.import = function (newconfig) {
        ...
        if (typeof browser !== "undefined") {
            for (const name in config.hotkeys) {
                browser.commands.update({name: name, shortcut: config.hotkeys[name]})
            }
        }
        ...
        if (typeof browser !== "undefined") {
            for (const name of Object.keys(chrome.runtime.getManifest().commands || {})) {
                ...
                if (info.suggested_key && info.suggested_key.default) {
                    browser.commands.update({name: name, shortcut: info.suggested_key.default})
                } else {
                    browser.commands.update({name: name, shortcut: ""})
                }
            }
        }
